...
export default function BulkListingUpload({ isOpen, onClose, onUploadComplete }: BulkListingUploadProps) {
  ...
  const validateListingData = (data: any[]): { validationErrors: ValidationError[], fieldMappings: FieldMapping[] } => {
    ...
    // Process Field,Value CSV format and parse addresses
    const processedData = processFieldValueCSV(data)
    ...
    processedData.forEach((row, index) => {
      ...
      // Check photos requirement (minimum 4 photos) - but only if PhotoURLs field exists
  ...
  const processUpload = async () => {
    ...
    try {
      ...
      // Validate real data using enhanced fuzzy field mapping
      ...
      // Process the data with address parsing
      const processedData = processFieldValueCSV(data)
      ...
      const draftListing: DraftListing = {
        ...
        data: processedData, // Use processed data with address parsing
        validationErrors,
      ...
      console.log('📤 Upload complete:', draftListing)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload MLS export files with intelligent field mapping. System detects {MLS_FIELD_DEFINITIONS.length} field variations including PropertyType, ArchitecturalStyle, ParcelID, and all feature fields.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              <strong>Enhanced Field Mapping:</strong> System now detects PropertyType, PropertySubtype, ArchitecturalStyle, 
              ParcelID, and all feature fields from Field,Value CSV format. Street addresses are automatically parsed 
              into StreetNumber, StreetName, and StreetSuffix using USPS suffix detection.
            </AlertDescription>
          </Alert>
