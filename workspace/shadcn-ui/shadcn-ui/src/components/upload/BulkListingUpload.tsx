...
export default function BulkListingUpload({ isOpen, onClose, onUploadComplete }: BulkListingUploadProps) {
  ...
  const readFileData = (file: File): Promise<Record<string, unknown>[]> => {
  ...
  const validateListingData = (data: Record<string, unknown>[]): { validationErrors: ValidationError[], fieldMappings: FieldMapping[] } => {
    ...
    data.forEach((row, index) => {
      ...
      if (photoMappings.length > 0) {
        ...
        if (!photos || (photos && String(photos).split(',').length < 4)) {
          validationErrors.push({
            row: index + 1,
            field: 'PhotoURLs',
            type: 'photos',
      ...
      if (mlsMappings.length > 0) {
        ...
        const mlsValue = row[mlsField]
        if (mlsValue && String(mlsValue).trim().length < 3) {
          validationErrors.push({
            row: index + 1,
            field: 'MLSNumber',
            type: 'format',
      ...
      const draftListing: DraftListing = {
        ...
        photosCount: data.reduce((sum, row) => {
          ...
          if (photoMappings.length > 0) {
            const photoField = photoMappings[0].inputField
            const photos = row[photoField]
            return sum + (photos ? String(photos).split(',').length : 0)
          }
          return sum
        }, 0),
