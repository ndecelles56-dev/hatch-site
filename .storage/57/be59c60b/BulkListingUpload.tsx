...
const parseCSV = (csvText: string): Record<string, any>[] => {
  ...
  if (firstLine.includes('field') && firstLine.includes('value')) {
    ...
    console.log('📋 Processing Field,Value pairs:')
    ...
    console.log('📋 All fields found:', Object.keys(listing))
...
export default function BulkListingUpload({ isOpen, onClose, onUploadComplete }: BulkListingUploadProps) {
  ...
  const validateListingData = (data: any[]): { validationErrors: ValidationError[], fieldMappings: FieldMapping[] } => {
    ...
    console.log('🔍 Starting Enhanced Field Mapping Process')
    console.log(`📋 Headers found in data: ${headers.length}`)
    headers.forEach((header, index) => console.log(`  ${index + 1}. "${header}"`))
    
    // Use enhanced fuzzy field matching to map headers with lower threshold for better matching
    const mappingResult = mapCSVHeaders(headers, 0.5) // Even lower threshold for better detection
    ...
    mappingResult.mappings.forEach((mapping, index) => {
      console.log(`  ${index + 1}. "${mapping.inputField}" → ${mapping.mlsField.standardName} (${Math.round(mapping.confidence * 100)}% confidence, ${mapping.isRequired ? 'Required' : 'Optional'})`)
    })

    if (mappingResult.unmapped.length > 0) {
      console.log('❌ UNMAPPED FIELDS:')
      mappingResult.unmapped.forEach((field, index) => console.log(`  ${index + 1}. "${field}"`))
    }

    if (mappingResult.missingRequired.length > 0) {
      console.log('⚠️ MISSING REQUIRED FIELDS:')
      mappingResult.missingRequired.forEach((field, index) => console.log(`  ${index + 1}. ${field.standardName}`))
    }

    // Process each record with enhanced processing
    data.forEach((row, index) => {
      console.log(`🔄 Processing record ${index + 1}`)
      console.log(`📋 Original record keys:`, Object.keys(row))
      
      // Apply enhanced processing including address parsing
      const processedRecord = processMLSRecord(row, fieldMappings)
      
      console.log(`📋 Processed record keys:`, Object.keys(processedRecord))
      
      // Update the original record with processed data
      Object.assign(row, processedRecord)
      
      console.log(`✅ Final record keys:`, Object.keys(row))
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload MLS export files with intelligent field mapping and address parsing. System detects {MLS_FIELD_DEFINITIONS.length} field variations including PropertyType, ArchitecturalStyle, ParcelID, and all feature fields.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              • PropertyType, ArchitecturalStyle, ParcelID now properly detected
              • Improved address parsing with USPS suffix detection
              • All feature fields (Flooring, Pool, Fireplace, Kitchen, etc.) supported
              • Field,Value CSV format with intelligent fuzzy matching
              • No fake photo generation - only processes actual data
            </AlertDescription>
          </Alert>
        ...
        <DialogFooter>
          ...
          >
            {isProcessing ? 'Processing with Enhanced Detection...' : 'Process Export'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
