...
// Enhanced CSV parser for Field,Value format
...
const processMLSRecord = (record: Record<string, any>, fieldMappings: FieldMapping[]): Record<string, any> => {
  const processedRecord = { ...record }
  
  console.log('🔄 Processing MLS record with enhanced features...')
  
  // Handle address parsing if full address is available
  const fullAddressValue = extractFieldValue(record, 'FullStreetAddress') || 
                           extractFieldValue(record, 'street address') ||
                           extractFieldValue(record, 'address') ||
                           extractFieldValue(record, 'property address')
  
  if (fullAddressValue) {
    console.log(`🏠 Parsing address: "${fullAddressValue}"`)
    const addressParts = parseStreetAddress(fullAddressValue)
    
    // Add parsed address components if not already present
    if (!extractFieldValue(record, 'StreetNumber') && addressParts.streetNumber) {
      processedRecord['Street Number'] = addressParts.streetNumber
      console.log(`  📍 Extracted Street Number: ${addressParts.streetNumber}`)
    }
    if (!extractFieldValue(record, 'StreetName') && addressParts.streetName) {
      processedRecord['Street Name'] = addressParts.streetName
      console.log(`  🛣️ Extracted Street Name: ${addressParts.streetName}`)
    }
    if (!extractFieldValue(record, 'StreetSuffix') && addressParts.streetSuffix) {
      processedRecord['Street Suffix'] = addressParts.streetSuffix
      console.log(`  🏷️ Extracted Street Suffix: ${addressParts.streetSuffix}`)
    }
  }
  
  // Enhanced feature field extraction using fuzzy matching
  const featureFields = [
    'ArchitecturalStyle', 'Flooring', 'PoolFeatures', 'FireplaceFeatures',
    'KitchenFeatures', 'PrimarySuite', 'LaundryFeatures', 'ConstructionMaterials',
    'Roof', 'FoundationDetails', 'ExteriorFeatures', 'View', 'WaterSource',
    'Sewer', 'HeatingType', 'CoolingType'
  ]
  
  for (const featureField of featureFields) {
    const fieldDef = MLS_FIELD_DEFINITIONS.find(f => f.standardName === featureField)
    if (fieldDef) {
      // Try to find the field using fuzzy matching
      for (const variation of fieldDef.variations) {
        const value = extractFieldValue(record, variation, 0.6) // Lower threshold for features
        if (value && value !== '') {
          processedRecord[featureField] = value
          console.log(`  ✨ Extracted ${featureField}: ${value}`)
          break
        }
      }
    }
...
export default function BulkListingUpload({ isOpen, onClose, onUploadComplete }: BulkListingUploadProps) {
  ...
  const validateListingData = (data: any[]): { validationErrors: ValidationError[], fieldMappings: FieldMapping[] } => {
    ...
    // Process each record with enhanced features
    const processedData = data.map(record => processMLSRecord(record, []))
  ...
  const processUpload = async () => {
    ...
    try {
      ...
      // Process data with enhanced features (address parsing, feature extraction)
      const processedData = rawData.map(record => processMLSRecord(record, []))
      console.log('🔄 Enhanced processing complete for', processedData.length, 'records')

      // Validate processed data using enhanced fuzzy field matching
      const { validationErrors, fieldMappings } = validateListingData(processedData)
      ...
      const validRecords = processedData.length - new Set(requiredErrors.map(e => e.row)).size
      const requiredFieldsComplete = processedData.length - new Set(requiredErrors.map(e => e.row)).size
      const optionalFieldsComplete = processedData.length - new Set(optionalErrors.map(e => e.row)).size

      setUploadProgress(80)
      await new Promise(resolve => setTimeout(resolve, 300))

      // Create enhanced draft listing from processed data
      ...
      console.log('🎉 Enhanced MLS upload complete:', {
        fileName: selectedFile.name,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload MLS export files with intelligent field mapping, address parsing, and feature extraction. System detects {MLS_FIELD_DEFINITIONS.length} field variations.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <Alert>
            ...
            <AlertDescription>
              <strong>Enhanced Processing:</strong> System now includes automatic address parsing (Street Number, Street Name, Street Suffix), 
              enhanced feature detection (Architectural Style, Flooring, Pool Features, Fireplace Features, Kitchen Features, Primary Suite, 
              Laundry Features, Construction Materials, Roof Type, Foundation Details, Exterior Features, View, Water Source, Sewer System, 
              Heating Type, Cooling Type), and Parcel ID mapping. Field,Value CSV format fully supported.
            </AlertDescription>
          </Alert>
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Supported formats: CSV (Field,Value), Excel (.xlsx, .xls) • Max size: 10MB • Max records: 50
              </p>
            </div>
          </div>
